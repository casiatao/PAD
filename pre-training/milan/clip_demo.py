    ...
    def forward(self, x: torch.Tensor):
        ...
        for idx, blk in enumerate(self.resblocks):
            if idx < self.layers-1:
                x = blk(x)
                z.append(x.permute(1,0,2))  # (L, N, E) --> (N, L, E)
            else:
                x, attention = blk(x)
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, finetune=False, average_targets=1):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
...
class CLIP(nn.Module):
    ...
    def initialize_parameters(self):
        ...
        if isinstance(self.visual, ModifiedResNet):  # ModifiedResNet在CLIP仓库里有
    ...
    def build_attention_mask(self):
        ...
        mask.triu_(1)  # zero out the lower diagonal  下三角置为0，即只可以与自己和在自己前面的token计算注意力
        return mask
