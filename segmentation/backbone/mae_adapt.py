    ...
    def __init__(self,
        ...
        if patch_wise_scalar:
            ...
            if adapter_scalar == "learnable_scalar":  
                ...
                if init_value != "0.0":
                    self.register_buffer('scale', torch.ones(1) * float(init_value))
                else:
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 window_size=None, attn_head_dim=None, down_size=64, adapt_scalar="frozen", init_value="0.0", layernorm_option="in",
    ...
    def forward(self, x, rel_pos_bias=None):
        if self.gamma_1 is None:
            ...
            if self.patch_wise_scalar:
                if self.fusion_method == 'concat':
                    scalar = self.scalar_pred(torch.cat([origin_mlp_x, adapt_x], axis=-1))
            ...
            adapt_x = adapt_x * scalar
            x = x + adapt_x
            x = x + self.drop_path(self.gamma_2 * origin_mlp_x)
        return x
...
class MAE_adapt(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
                 ...
                 down_size=64, adapt_scalar="frozen", init_value="0.0", layernorm_option="in",
                 patch_wise_scalar=False, fusion_method="concat"):
        super().__init__()
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        self.adapt_down_size = down_size
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                down_size=down_size, adapt_scalar=adapt_scalar, init_value=init_value, layernorm_option=layernorm_option,
